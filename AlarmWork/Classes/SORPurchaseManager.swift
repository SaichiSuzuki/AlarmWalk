...
private let purchaseManagerSharedManager = SORPurchaseManager()

class SORPurchaseManager : NSObject,SKPaymentTransactionObserver {
    
    var delegate : SORPurchaseManagerDelegate?
    
    private var productIdentifier : String?
    private var isRestore : Bool = false
    
    /// シングルトン
    class func sharedManager() -> SORPurchaseManager{
...
@objc protocol SORPurchaseManagerDelegate {
    //課金完了
    optional func purchaseManager(purchaseManager: SORPurchaseManager!, didFinishPurchaseWithTransaction transaction: SKPaymentTransaction!, decisionHandler: ((complete : Bool) -> Void)!)
    //課金完了(中断していたもの)
    optional func purchaseManager(purchaseManager: SORPurchaseManager!, didFinishUntreatedPurchaseWithTransaction transaction: SKPaymentTransaction!, decisionHandler: ((complete : Bool) -> Void)!)
    //リストア完了
    optional func purchaseManagerDidFinishRestore(purchaseManager: SORPurchaseManager!)
    //課金失敗
    optional func purchaseManager(purchaseManager: SORPurchaseManager!, didFailWithError error: NSError!)
    //承認待ち(ファミリー共有)
    optional func purchaseManagerDidDeferred(purchaseManager: SORPurchaseManager!)
}